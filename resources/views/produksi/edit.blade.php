...
@section('content')
    ...
    <form action="{{ route('produksi.update', $record) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="monitoring-form-card">
            <div class="monitoring-form-header">
                <div class="monitoring-form-header-icon"
                    style="background: rgba(8, 145, 178, 0.1); color: var(--kkp-teal);">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:20px;height:20px;">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="monitoring-form-body">
                <div class="grid grid-cols-3">
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div class="form-control readonly-control" style="cursor:default;">{{ $record->kdmp->nama_kdkmp }}
                        </div>
                    </div>
                    <div class="form-group" style="margin-bottom:0;">
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div class="form-control readonly-control" style="cursor:default;">{{ $record->kdmp->kabupaten }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="monitoring-form-body">
                <div class="grid grid-cols-3">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control rupiah-input" data-target="biaya_pakan"
                            value="{{ old('biaya_pakan', $record->biaya_pakan) }}" inputmode="numeric">
                        <input type="hidden" name="biaya_pakan" value="{{ old('biaya_pakan', $record->biaya_pakan) }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Biaya Bibit (Rp)</label>
                        <input type="text" class="form-control rupiah-input" data-target="biaya_bibit"
                            value="{{ old('biaya_bibit', $record->biaya_bibit) }}" inputmode="numeric">
                        <input type="hidden" name="biaya_bibit" value="{{ old('biaya_bibit', $record->biaya_bibit) }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Biaya Lainnya (Rp)</label>
                        <input type="text" class="form-control rupiah-input" data-target="biaya_lainnya"
                            value="{{ old('biaya_lainnya', $record->biaya_lainnya) }}" inputmode="numeric">
                        <input type="hidden" name="biaya_lainnya"
                            value="{{ old('biaya_lainnya', $record->biaya_lainnya) }}">
                    </div>
                </div>
                <div class="grid grid-cols-3 mt-2">
                    <div class="form-group">
                        <label class="form-label">Volume Panen (kg)</label>
                        <input type="text" class="form-control rupiah-input" data-target="volume_panen_kg"
                            data-decimal="true" value="{{ old('volume_panen_kg', $record->volume_panen_kg) }}"
                            inputmode="decimal">
                        <input type="hidden" name="volume_panen_kg"
                            value="{{ old('volume_panen_kg', $record->volume_panen_kg) }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Nilai Produksi (Rp)</label>
                        <input type="text" class="form-control rupiah-input" data-target="nilai_produksi"
                            value="{{ old('nilai_produksi', $record->nilai_produksi) }}" inputmode="numeric">
                        <input type="hidden" name="nilai_produksi"
                            value="{{ old('nilai_produksi', $record->nilai_produksi) }}">
                    </div>
                    <div class="form-group" style="margin-bottom:0;">
                        <label class="form-label">Jumlah Pembudidaya Aktif</label>
                        <input type="text" class="form-control rupiah-input" data-target="jumlah_pembudidaya_aktif"
                            value="{{ old('jumlah_pembudidaya_aktif', $record->jumlah_pembudidaya_aktif) }}"
                            inputmode="numeric">
                        <input type="hidden" name="jumlah_pembudidaya_aktif"
                            value="{{ old('jumlah_pembudidaya_aktif', $record->jumlah_pembudidaya_aktif) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="monitoring-form-body">
                ...
                <div class="form-group">
                    ...
                    <textarea name="tindak_lanjut" rows="3" class="form-control"
                        placeholder="Rencana tindak lanjut...">{{ old('tindak_lanjut', $record->tindak_lanjut) }}</textarea>
                </div>
                <div class="form-group" style="margin-bottom:0;">
                    <label class="form-label">Catatan Tambahan</label>
                    <textarea name="catatan" rows="2" class="form-control"
                        placeholder="Catatan lain...">{{ old('catatan', $record->catatan) }}</textarea>
                </div>
            </div>
        </div>
